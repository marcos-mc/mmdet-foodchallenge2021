...
predictions_file_path = r'/home/marcosmejia/UB/Github/mmdetection/predictions.segm.json'
analysis_file_path = r'./test_analysis.csv'
...
for category in COCO(ground_truth_path).getCatIds():
    ...
    cocoEval.params.catIds = [category]
    cocoEval.evaluate()
    cocoEval.accumulate()
    ...
    class_wise_results.append([category, average_precision, average_recall])

with open(analysis_file_path, 'w') as f_obj:
    csv_writer = csv.writer(f_obj)
    for cw in class_wise_results:
        csv_writer.writerow(cw)
